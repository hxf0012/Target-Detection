...
def load_ans(data_path_, rpos_path_, fs_):
    ...
    for rpos_file in rpos_files:
        ...
        ref_path = os.path.join(rpos_path_, rpos_file) # ref文件
        ecg_path = os.path.join(data_path_, ecg_file) # ecg文件

        ecg_data = np.transpose(sio.loadmat(ecg_path)['ecg'])[0]  # ecg文件里内容
        r_ref = sio.loadmat(ref_path)['R_peak'].flatten()   # ref文件里内容
        r_ref = r_ref[(r_ref >= 0.5*fs_) & (r_ref <= 9.5*fs_)]
        # print('#################',r_ref)

        r_hr = np.array([loc for loc in r_ref if (loc > 5.5 * fs_ and loc < len(ecg_data) - 0.5 * fs_)])
        hr_ans, r_ans = CPSC2019_challenge(ecg_data,index)
        r_ans = np.array(r_ans)  # list转换为array
...
def score(r_ref, hr_ref, r_ans, hr_ans, fs_, thr_):
    print('原qrs:', r_ref)  # 原qrs
    print('原心率:', hr_ref)  # 原心率
    print('测试qrs:', r_ans)  # 测试qrs
    print('测试心率:', hr_ans)  # 测试心率
...
if __name__ == '__main__':
    ...
    THR = 0.075
    #单张
    # DATA_PATH = '/VOCdevkit/VOC2007-ecg/ecg_data/rcnn/data/'
    # RPOS_PATH = '/VOCdevkit/VOC2007-ecg/ecg_data/rcnn/ref/'
    # 批量
    DATA_PATH = '/VOCdevkit/VOC2007-ecg/ecg_data/qrs_1/data/'
    RPOS_PATH = '/VOCdevkit/VOC2007-ecg/ecg_data/qrs_1/ref/'
